...
The HackStation menu system has been reorganized into category-based submenus.
...
- Forensics / Artifact
- System / Output / Backup

This makes the interface easier to extend and turns HackST into a more structured learning and field-work environment.
...
Note: package availability may vary depending on the current platform and architecture. Alpine is currently used as a test platform, while the long-term target is Artix / Arch Linux.

## Design goals

HackST is not just a launcher for security tools.

It is being developed as:

- an interactive menu-based toolkit
- a learning-oriented environment
- a reusable field station for local audits, diagnostics and analysis
- a modular shell-based framework that can be migrated across systems

## Output handling

Generated logs and command outputs are stored under:

`/root/outputs/`

System-level backups are handled separately through the local backup workflow.

## Notes

Some modules may work differently depending on the host distribution, CPU architecture, and package availability.

The current development and testing platform is Alpine Linux on ARMHF.
The intended long-term migration target is Artix / Arch Linux.
